...
def initial_hyperparameter(maxx, minx,dim):
    rnd = hp.rand
...
class TD3():
    def __init__(self):
        ...
        self.memory_capacity = hp.memory_size
    ...
    def train(self, states, actions, next_states, rewards, done, tau, discount, weights=None):
        if weights is None:
            weights = np.ones_like(rewards)
        actor_loss, critic_loss, td_errors = self._train_body(
            states, actions, next_states, rewards, done, weights, tau, discount)

        return actor_loss, critic_loss, td_errors

    @tf.function
    def _train_body(self, states, actions, next_states, rewards, done, weights, tau, discount):

        with tf.device(self.device):
            with tf.GradientTape(persistent=True) as tape:
                td_error1, td_error2 = self._compute_td_error_body(states, actions, next_states, rewards, done, discount)
            ...
            if tf.math.equal(self._it % self._actor_update_freq, 0):
                ...
                update_target_variables(
                    self.critic_target.weights, self.critic.weights, tau)
                update_target_variables(
                    self.critic_2_target.weights, self.critic_2.weights, tau)

                update_target_variables(
                    self.actor_target.weights, self.actor.weights, tau)

            return actor_loss, critic_loss, tf.abs(td_error1) + tf.abs(td_error2)


    @tf.function
    def _compute_td_error_body(self, states, actions, next_states, rewards, dones, discount):
        with tf.device(self.device):
            not_dones = 1. - dones
            # Get noisy action
            ...
            taret_Q = tf.stop_gradient(target_Q)
...
class Actor(tf.keras.Model):
    def __init__(self, state_shape, action_dim, max_action, name="Actor"):
        ...
        self.l1 = Dense(hp.actor_l1, name="L1")
        self.l2 = Dense(hp.actor_l2, name="L3")
...
class Critic(tf.keras.Model):
    def __init__(self, state_shape, action_dim, name="Critic"):
        ...
        self.l1 = Dense(hp.critic_l1, name="L1")
        self.l2 = Dense(hp.critic_l2, name="L2")
...
class Trainer:
    def __init__(self, policy, env):
        ...
        # experiment settings
        self._max_steps = hp.max_step
        self._max_episode = hp.max_episode
        self._episode_max_steps = hp.max_time_step
    ...
    def __call__(self):
        ...
        dim = hp.dim
        fitness_best = hp.fitness_best
        tau = hp.tau
        discount = hp.discount_factor
        Xcurrent, Xbest = initial_hyperparameter(hp.maxx, hp.minx, dim)
        #tau = Xcurrent[0]
        #discount = Xcurrent[1]
        random_action_prob = 1/Xcurrent[0]
        batch_size = Xcurrent[1]
        rnd = hp.rand
        ...
        h_score = []
        h_success = []
        while(n_episode<self._max_episode):
            ...
            if episode_return > fitness_best:
                Xbest, Xcurrent = Xcurrent, Xbest

            if done:
                a=2*(1-n_episode/self._max_episode)
                a2=-1+n_episode*((-1)/self._max_episode)
                ...
                for j in range(dim):
                    ...
                    if p < 0.5:
                        if abs(A)>1:
                            ...
                            Xcurrent[j] = int(Xbest[j] - A * D[j])
                        else:
                            Xcurrent[j] = int(((hp.maxx[j] - hp.minx[j]) * rnd.random() + hp.minx[j]))  		
                            
                    else:	
                        D1[j] = abs(Xbest[j] - Xcurrent[j])
                        Xcurrent[j] = int(D1[j] * math.exp(b * l) * math.cos(2 * math.pi * l) + Xbest[j])	

                    Xcurrent[j] = max(Xcurrent[j], hp.minx[j])
                    Xcurrent[j] = min(Xcurrent[j], hp.maxx[j])
                ...
                if total_steps > self._policy.n_warmup:
                    ...
                    actor_loss, critic_loss, _ = self._policy.train(samples["obs"], samples["act"], samples["next_obs"], samples["rew"],
                                                                    np.array(samples["done"], dtype=np.float32), tau, discount,)
                    with self.train_summary_writer.as_default():
